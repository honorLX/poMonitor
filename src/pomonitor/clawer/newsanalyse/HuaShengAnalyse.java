...
public class HuaShengAnalyse extends BaseAnalyse {
	...
	public int getPageCount(String key,boolean isLatest) {
		...
		try {
			...
			//用文章类名得到文章总数
			Elements listEle= doc.getElementsByAttributeValue("class", "support-text-top");
			String countStr=listEle.text();
			if(countStr.length()<12)
			{
				countStr=countStr.substring(countStr.length()-3, countStr.length()-1);
			}else{
				countStr=countStr.substring(countStr.length()-4, countStr.length()-1);
			}
	...
	public HashMap<String, Object> analyseAnyPage(String Strurl) {
		...
		try {
			...
			//取出每一条的内容
			Elements listEle= doc.getElementsByAttributeValue("class", "result f s0");
			for(Element e:listEle){
				...
				//取出title
				Elements titleE=e.getElementsByAttributeValue("class","c-title");
				String title=titleE.text();
				System.out.println("title:"+title);
				//取出time
				Elements timeE=e.getElementsByAttributeValue("class", "c-showurl");
				String time=timeE.text();
				time=time.substring(time.length()-10, time.length());
				System.out.println("time:"+time);
				String web="华声在线";
				//取出content
				Elements contentE=e.getElementsByAttributeValue("class", "c-abstract");
